...
def detector(LStr):
    # Detector bayesiano: detector + dirección (ab/je/up/dw)
    if not LStr:
        return False
    match LStr:
        case ["detector", dr]:
...
def asignacion(LStr):
    ...
    match LStr:
        ...
        # rX = abelardo dw
        case [A, "=", B, C]:
            return regs(A) and (sens([B, C]) or detector([B, C]))

        # rX = rY + 0xNN / rX = rY - 0xNN / rX = rY + rZ / rX = rY - rZ
        case [A, "=", B, OP, C]:
            if not regs(A) or not regs(B) or not oper(OP):
                return False
            return regs(C) or numhex(C)
